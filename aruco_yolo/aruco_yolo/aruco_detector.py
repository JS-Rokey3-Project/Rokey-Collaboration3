        ...
        if len(detect_data) == 0:
            ...
            for marker in marker_array_msg.markers:
                self.get_logger().info(
                    ...
                    f"Position: ({marker.pose.pose.position.x:.2f}, {marker.pose.pose.position.y:.2f}, {marker.pose.pose.position.z:.2f}), "
                    f"Orientation (Euler): Roll={marker.pose.pose.orientation.z:.2f}, "
                    f"Pitch={marker.pose.pose.orientation.y:.2f}, Yaw={marker.pose.pose.orientation.x:.2f}"
                )
